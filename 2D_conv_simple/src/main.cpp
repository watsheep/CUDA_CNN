...
__global__ void conv_forward_valid_kernel(float *X, int batch,int H_in,int W_in,int Channel_in,
  ...
  if(h<H_out&&w<W_out){
    float tmp = 0.0;
    int yoffset = ((i * H_out + h) * W_out + w) * M_out + m;

    for (const auto p : range(0, filter_h)) {
      for (const auto q : range(0, filter_w)) {
          for (const auto c : range(0, Channel_in)) {
            
            const auto xoffset = i * H_in * W_in * Channel_in +
                                       (h + p) * W_in * Channel_in +
                                       (w + q) * Channel_in + c;
            const auto woffset = p * filter_w * Channel_in * M_out +
                                       q * Channel_in * M_out + c * M_out + m;
            
            tmp += X[xoffset] * W[woffset];
...
void forward_operation(float *x, float *conv1, float *conv2, float *fc1,
  ...
  conv_forward_valid_kernel<<<con_gridDim,con_blockDim>>>(deviceX, xdims[0], xdims[1],xdims[2],xdims[3],
  ...
  conv_forward_valid_kernel<<<con_gridDim2,con_blockDim2>>>(deviceB, bdims[0], bdims[1],bdims[2],bdims[3],
